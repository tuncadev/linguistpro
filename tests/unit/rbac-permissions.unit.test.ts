...
describe("rbac protected API route matrix", () => {
  ...
  it("keeps cross-scope protected endpoints aligned with role policy", () => {
    const filesWithExpectedGuards: Array<{ file: string; guards: Set<Role>[] }> = [
      ...
      {
        file: path.join(projectRoot, "app", "api", "live-classes", "route.ts"),
        guards: [new Set<Role>(["STUDENT", "TUTOR", "ADMIN"]), new Set<Role>(["TUTOR", "ADMIN"])],
      },
      {
        file: path.join(projectRoot, "app", "api", "live-classes", "[id]", "join", "route.ts"),
        guards: [new Set<Role>(["STUDENT", "TUTOR", "ADMIN"])],
      },
      {
        file: path.join(projectRoot, "app", "api", "live-classes", "[id]", "remind", "route.ts"),
        guards: [new Set<Role>(["TUTOR", "ADMIN"])],
      },
      {
        file: path.join(projectRoot, "app", "api", "live-classes", "[id]", "cancel", "route.ts"),
        guards: [new Set<Role>(["TUTOR", "ADMIN"])],
      },
      {
        file: path.join(projectRoot, "app", "api", "admin", "communications", "send", "route.ts"),
